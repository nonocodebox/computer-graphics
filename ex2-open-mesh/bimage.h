...
public:
  ...
  const uchar * getImageData() const { return _data ; }
  ...
  inline uchar operator()(int x, int y, int col) const; //fast
...
inline unsigned char BImage::operator()(int x, int y, int col) const {
	return _data[(x + y * _width) * DEPTH + col];
}
